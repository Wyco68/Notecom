...
//   - The ONLY thing that mints a real session is `verifyOtp` on the 6-digit
...
//   - Codes, not magic links: the user types the code, so nothing ever follows
//     the email's link. That also sidesteps the project's Auth "Site URL"
//     (which points at another app) — a link click would redirect there; a
//     typed code never does.
//
// Email templates must expose {{ .Token }} for "Confirm signup", "Magic Link"
// and "Reset Password", and "Confirm email" must stay ON, or no code is sent.
...
export async function POST(req: NextRequest) {
  ...
  switch (action) {
    ...
    case "verify": {
      if (!email || !token) return bad("email and code are required");
    ...
    // --- reset step 1: email a recovery code (also how a magic-link-only
    //     account gets its first password) ---
    case "reset": {
      if (!email) return bad("email is required");
      // Errors are swallowed so a caller can't learn whether the email exists.
      await ephemeralClient().auth.resetPasswordForEmail(email).catch(() => {});
      return NextResponse.json({ ok: true, factor: "recovery" });
    }

    // --- reset step 2: verify the recovery code, then set the new password ---
    case "reset-verify": {
      if (!email || !token || password.length < 8) {
        return bad("email, code and a password of at least 8 characters are required");
      }
      const supabase = await createClient();
      const { error } = await supabase.auth.verifyOtp({ email, token, type: "recovery" });
      if (error) return bad("invalid or expired code", 401);
      const upd = await supabase.auth.updateUser({ password });
      if (upd.error) return bad("could not set the password", 400);
      return NextResponse.json({ ok: true });
    }
